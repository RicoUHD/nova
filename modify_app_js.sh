...
window.t = function(text) {
    if (!text) return text;
    if (i18nDict[text] && i18nDict[text] !== text) return i18nDict[text];

    if (text.startsWith("noch ") && text.includes(" Monat")) {
        const num = text.replace(/[^0-9]/g, '');
        return i18nDict["noch X Monate"] ? i18nDict["noch X Monate"].replace('X', num) : text;
    }
    if (text.includes(" Monate überfällig") || text.includes(" Monat überfällig")) {
        const num = text.replace(/[^0-9]/g, '');
        return i18nDict["X Monate überfällig"] ? i18nDict["X Monate überfällig"].replace('X', num) : text;
    }
    return text;
};
